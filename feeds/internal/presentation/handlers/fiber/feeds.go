...
type FeedsRouter struct {
	rec        usecases.FeedsUseCases
	commDealer contracts.CommentsDealer
}

func NewFeedRouter(uc usecases.FeedsUseCases, commDealer contracts.CommentsDealer) FeedsRouter {
	return FeedsRouter{uc, commDealer}
}
...
func (fr FeedsRouter) GetFeeds(ctx fiber.Ctx) error {
	...
	_, err := fr.rec.GetFeeds(cto, req, &fBuf)
	if err == nil {
		gRes.Code = errro.Success
		gRes.Message = "fetch feeds success"
		res := dto.GetFeedsResponse{GenericResponse: gRes, Details: fBuf.Value}
