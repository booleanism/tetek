...
type FeedsContr struct {
	...
	uc  usecases.FeedsUseCases
}

func NewFeeds(con *amqp091.Connection, uc usecases.FeedsUseCases) *FeedsContr {
	return &FeedsContr{con, uc}
}
	...
	go func() {
		for d := range mgs {
			...
			func() {
				...
				if task.Cmd == 0 {
					gfr := dto.GetFeedsRequest{ID: task.ID, Type: task.Type}
					n, err := c.uc.GetFeeds(ctx, gfr, &fBuf)
					if err == nil {
						res, _ := json.Marshal(&amqp.FeedsResult{Code: errro.Success, Message: "feeds found", Details: fBuf.Value[:n]})
						feedsResultPublisher(log, task, ch, d, res, err)
						return
					}

					if err.Code() == errro.ErrFeedsNoFeeds {
						res, _ := json.Marshal(&amqp.FeedsResult{Code: errro.ErrFeedsNoFeeds, Message: "feeds not found", Details: []entities.Feed{{ID: gfr.ID}}})
						feedsResultPublisher(log, task, ch, d, res, err)
						return
					}

					res, _ := json.Marshal(&amqp.FeedsResult{Code: errro.ErrFeedsDBError, Message: "something happen in our end", Details: []entities.Feed{{ID: gfr.ID}}})
					feedsResultPublisher(log, task, ch, d, res, err)
					return
				}
