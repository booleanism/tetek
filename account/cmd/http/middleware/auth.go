...
func Auth(auth *contract.LocalAuthContr) fiber.Handler {
	return func(ctx fiber.Ctx) error {
		...
		if req.Authorization == "" {
			res := authResponse{
				GenericResponse: helper.GenericResponse{
					Code:    errro.ErrAuthMissingHeader,
		...
		if !ok {
			res := authResponse{
				GenericResponse: helper.GenericResponse{
					Code:    errro.ErrAuthMissmatchScheme,
		...
		if err := auth.Publish(id, task); err != nil {
			res := authResponse{
				GenericResponse: helper.GenericResponse{
					Code:    errro.ErrServiceUnavailable,
					Message: "auth service unavailable",
				},
			}
			e := errro.New(res.Code, res.Message)
			return e.WithDetail(res.Json(), errro.TDetailJSON).SendError(ctx, fiber.StatusServiceUnavailable)
		}
		...
		res := authResponse{
			GenericResponse: helper.GenericResponse{
				Code:    errro.ErrAuthJWTVerifyFail,
				Message: "authorization failed",
			},
		}
		e := errro.New(res.Code, res.Message)
		return e.WithDetail(res.Json(), errro.TDetailJSON).SendError(ctx, fiber.StatusInternalServerError)
	}
}
