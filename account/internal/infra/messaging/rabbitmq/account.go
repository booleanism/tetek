...
type AccContr struct {
	...
	uc  usecases.GetUserProfileUseCase
}

func NewAccount(con *amqp091.Connection, uc usecases.GetUserProfileUseCase) *AccContr {
	return &AccContr{con, uc}
}
...
func (c *AccContr) WorkerAccountListener(ctx context.Context) (*amqp091.Channel, error) {
	...
	err = messaging.AccountSetup(ch)
	if err != nil {
		log.Error(err, "failed to setup account topic")
		return nil, err
	}

	mgs, err := ch.Consume(messaging.AccountTaskQueue, "", false, false, false, false, nil)
	if err != nil {
	...
	go func() {
		for d := range mgs {
			...
			task := messaging.AccountTask{}
			err := json.Unmarshal(d.Body, &task)
			if err != nil {
				log.V(1).Info("failed to marshal account task", "requestID", d.CorrelationId, "error", err, "body", d.Body)
				res, _ := json.Marshal(&messaging.AccountResult{Code: errro.ErrAccountParseFail, Message: "error parsing account task"})
			...
			if task.Cmd == 0 {
				u := &task.User
				gpr := dto.ProfileRequest{Uname: u.Uname}
				err := c.uc.GetProfile(ctx, gpr, &u)
				if err == nil {
					res, _ := json.Marshal(&messaging.AccountResult{Code: errro.Success, Message: "user found", Detail: *u})
					accountResultPublisher(log, task, ch, d, res, err)
					continue
				}

				if err.Code() == errro.ErrAccountNoUser {
					res, _ := json.Marshal(&messaging.AccountResult{Code: errro.ErrAccountNoUser, Message: "user not found", Detail: task.User})
					accountResultPublisher(log, task, ch, d, res, err)
					continue
				}

				res, _ := json.Marshal(&messaging.AccountResult{Code: errro.ErrAccountDBError, Message: "something happen in our end", Detail: task.User})
				accountResultPublisher(log, task, ch, d, res, err)
				continue
			}

			res, _ := json.Marshal(&messaging.AccountResult{Code: errro.ErrCommUnknownCmd, Message: "unknown command"})
...
func accountResultPublisher(log logr.Logger, task any, ch *amqp091.Channel, d amqp091.Delivery, res []byte, e error) {
	...
	err := ch.Publish(messaging.AccountExchange, messaging.AccountResRk, false, false, amqp091.Publishing{
		CorrelationId: d.CorrelationId,
		Body:          res,
		ContentType:   "text/json",
	})
	if err != nil {
		log.Error(err, "failed to publish account result", "result", res, "exchange", messaging.AccountExchange, "routing-key", messaging.AccountResRk)
